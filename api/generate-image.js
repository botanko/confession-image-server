...
// Global storage for images (in production, use a proper database)
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      // Store image in memory (in production, use a proper database or cloud storage)
      global.imageStorage[imageId] = imageBuffer;
      
      // Clean up old images (keep only last 100)
      const keys = Object.keys(global.imageStorage);
      if (keys.length > 100) {
        const oldKeys = keys.slice(0, keys.length - 100);
        oldKeys.forEach(key => delete global.imageStorage[key]);
      }
      
      // Return both base64 and URL options
      const base64Image = imageBuffer.toString('base64');
      const imageUrl = `${req.headers['x-forwarded-proto'] || 'https'}://${req.headers.host}/api/image/${imageId}`;
      
      return res.status(200).json({
        success: true,
        image: `data:image/png;base64,${base64Image}`,
        imageUrl: imageUrl, // New: Direct URL for Instagram
        imageId: imageId,
        timestamp: new Date().toISOString()
      });
